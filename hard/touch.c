...
//	temp=0.065317*10000000;
	...
	//保存x偏移量
    EE_Write_Word(XOFF,tp_dev.xoff);	    
	//保存y偏移量
...
{					  
	long tempfac;
	tempfac=EE_Read_Byte(TOUCHFLAG);//读取标记字,看是否校准过！ 		 
	...
	{    			
		...
		tp_dev.xoff=EE_Read_Word(XOFF);			   	  
 	    //得到y偏移量
		tp_dev.yoff=EE_Read_Word(YOFF);				 	  
 		tp_dev.touchtype=EE_Read_Byte(TOUCHTYPE);//读取触屏类型标记
